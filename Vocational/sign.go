...
func (c *info) GetDate(date string) {
	...
	// NewGetStuFaceActivityList(c.UserInfo.UserID, c.today.DataList[0].ID, c.today.DataList[0].OpenClassID, c.LoginInfo.schoolId)
...
func (i *info) NewGetStuFaceActivityList() {
	...
	for _, v := range i.Today.DataList {
		...
		data := map[string]string{
			"activityId":  v.ID,
			"stuId":       i.UserInfo.UserID,
			"classState":  "2",
			"openClassId": v.OpenClassID,
			"newToken":    i.UserInfo.NewToken,
		}
		resp, err := req.SetHeaders(header).SetFormData(data).Post(url)
		if err != nil {
			panic(err)
		...
		err = resp.Unmarshal(&i.Classroom)
		if err != nil {
			return
		}
	...
	for _, n := range c.Classroom.DataList {
		if n.DataType == "签到" && n.State != 1 {
			...
			if tamp.IsAttend != 1 {
				...
				_, err = req.SetHeaders(header).SetFormData(data).Post("https://zjyapp.icv.com.cn/newmobileapi/faceteach/saveStuSignNew")
				// 这里发送一个信息通知
				// fmt.Println(s)
